...
- Audio is processed in RAM: a 0.5-second pre-roll, at most 15 seconds per decoding segment, and a 20-second bounded capture backlog. Capture/VAD run independently of Whisper decoding. Partial snapshots are requested every ~0.75 seconds of audio; obsolete queued partials are replaced, while up to four queued final/partial snapshots retain segment order. The sphere receives microphone RMS updates every ~50 ms rather than a fixed animation level. Buffers are released/reset after stop or cancellation. Overflow pauses listening with an error rather than silently dropping commands. A command is limited to 120 seconds / 8,000 characters and is never automatically sent on reaching that limit.
