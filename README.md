...
The examples below use the English defaults: agent name `Brute`, recognition language English (US), new-session phrases `new session, start new session`, and end phrases `over, end command`. All phrase lists are editable as comma-separated values; previously saved values are preserved.

- Say the name before **every** command: `Brute, check the tests, over`.
...
- Finish with a configured suffix (`over`, `end command` by default), the Send button, or 1.5 seconds without detected speech (adjustable 0.5–30 seconds). The old unversioned 10-second default migrates to 1.5 seconds; other saved delays are preserved. A newly saved explicit 10-second delay is preserved too. Send waits until pending audio has been recognized.
- `Brute, new session, over` starts a fresh context for the next command. `Brute, start new session, check memory, end command` immediately submits to a new context. New-session phrases are recognized only at the start of a command; end phrases are recognized only at the end. `Brute, cancel` discards the active utterance. Reset without a task is rejected while the current backend turn is running; a new-session task can be queued.
